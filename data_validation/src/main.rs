...
fn main() {
    ...
    loop {
        ...
        match input("Your choice").as_str() {
            ...
            "2" => {
                let phone = input("Enter phone number (e.g. +234810007776): ");
            ...
            "3" => {
                ...
                if is_strong_password(&pwd) {
                    ...
                    println!(
                        "❌ Weak password (must be 8+ chars, contain upper, lower, digit, and symbol)."
                    )
                }
            }
...
fn is_strong_password(pwd: &str) -> bool {
    let re = Regex::new(r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$").unwrap();
    re.is_match(pwd)
}
