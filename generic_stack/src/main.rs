...
impl<T> Stack<T> {
    ...
    fn pop (&mut self) -> Option<T> {    //Removes and returns the top item 
        self.elements.pop()
    }

    fn peek(&self) -> Option<&T> {    // Returns a reference to the top item without removing it.
        self.elements.last()
    }
